...
class VideoProcessorService:
    ...
    def extract_keyframes(self, video_path: str, max_duration_seconds: Optional[float] = None) -> List[tuple]:
        """
        Extract keyframes from a visual based on scene changes, up to a specified duration.

        Args:
            video_path: Path to the visual file.
            max_duration_seconds: Maximum duration in seconds to extract keyframes from (None for the entire visual).

        Returns:
            List of tuples containing (frame_number, timestamp, frame).
        """
        # Open the visual file
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            raise ValueError(f"Could not open visual file: {video_path}")
        ...
        while True:
            ...
            if not ret or (max_frame and frame_number >= max_frame):
                break  # Stop if visual ends or max duration is reached
    ...
    def extract_hook_frame(self, video_path: str, frame_time: int = 1) -> Optional[np.ndarray]:
        """
        Extract a specific frame from a visual file

        Args:
            frame_time (int): Time to extract the frame (default: 1)
            video_path (str): Path to the visual file
        ...
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            print(f"Error opening visual file: {video_path}")
            return None
